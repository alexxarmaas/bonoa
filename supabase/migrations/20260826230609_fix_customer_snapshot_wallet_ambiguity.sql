-- PL/pgSQL RETURNS TABLE exposes output columns as variables. Qualify wallet_id
-- inside the UNION CTE so it cannot collide with the output variable.
...
begin
  ...
  with pass_stats as (
    ...
    select ps0.wallet_id from pass_stats ps0
    union
    select es0.wallet_id from event_stats es0
  ), combined as (
    select
      wb.wallet_id,
