-- Make automatic loyalty rewards self-healing: if a reward cannot be emitted at
-- the exact threshold, a later redemption will backfill every missing milestone.
...
begin
  ...
  if target_wallet_id is null or trigger_product_id is null then
    return new;
  end if;
  ...
  for rule_record in
    ...
    where r.business_id = new.business_id
      ...
      and r.active = true
  loop
    -- Serialize reward calculation for one rule/customer while still allowing
    -- unrelated customers and rules to progress concurrently.
    ...
    for candidate_milestone in 1..eligible_milestones loop
      begin
        -- The unique(rule_id, wallet_id, milestone) constraint is the final
        -- idempotency barrier, including concurrent redemption requests.
        grant_id := null;
        ...
        if reward_product.id is null then
          raise exception 'reward_product_unavailable';
        end if;
        ...
        insert into public.passes (
          ...
          reward_product.initial_units,
          case
            when reward_product.validity_days is null then null
            else now() + make_interval(days => reward_product.validity_days)
          end,
          0,
          reward_product.currency
        ) returning id into reward_pass_id;

        update public.loyalty_reward_grants
        set pass_id = reward_pass_id
        where id = grant_id;

        insert into public.business_audit_events (
          business_id, actor_id, event_type, pass_id, product_id, metadata
        ) values (
          new.business_id,
          new.performed_by,
          'reward_issued',
      ...
      exception when others then
        -- PL/pgSQL exception blocks roll back the candidate's grant/pass writes.
        -- Therefore a future redemption can safely retry this missing milestone.
        null;
      end;
    end loop;
  end loop;
