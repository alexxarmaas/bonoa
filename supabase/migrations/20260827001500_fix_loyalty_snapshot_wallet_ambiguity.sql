-- Disambiguate wallet_id inside the PL/pgSQL RETURNS TABLE function.
-- Output column names are PL/pgSQL variables, so unqualified wallet_id references
-- in the UNION can otherwise be interpreted as either a variable or a CTE column.
...
begin
  ...
  with pass_stats as (
    ...
    select
      g.wallet_id,
      count(*)::bigint as rewards_earned
