  ...
  return (
    <main className="bonoa-shell pb-24">
      ...
      <section id="bonos" className="mt-10">
        ...
        {loading ? <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{[0, 1, 2].map((item) => <div key={item} className="bonoa-card h-64 animate-pulse rounded-[1.6rem]" />)}</div> : error ? <div className="rounded-[1.6rem] border border-red-400/15 bg-red-400/5 p-6 text-sm text-red-200">{error}</div> : passes.length ? <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{passes.map((pass) => <PassCard key={pass.id} pass={pass} />)}</div> : <div className="bonoa-card rounded-[1.8rem] p-8 text-center sm:p-10"><div className="mx-auto grid h-14 w-14 place-items-center rounded-2xl border border-orange-400/15 bg-orange-400/[0.07] text-orange-300"><MdWallet size={27} /></div><h3 className="mt-5 text-xl font-black text-white">Tu wallet está lista</h3><p className="mx-auto mt-2 max-w-md text-sm leading-6 text-zinc-500">Todavía no tienes bonos. Muestra tu QR en un establecimiento asociado y aparecerán aquí en cuanto te asignen uno.</p><Link href="/qr" className="brand-gradient mt-5 inline-flex items-center gap-2 rounded-full px-5 py-3 text-xs font-black text-white"><MdQrCode2 size={18} /> Mostrar mi QR</Link></div>}
      </section>

      <p className="mt-10 text-center text-[11px] leading-5 text-zinc-600 md:text-left">BONŌA · Tus fidelizaciones se sincronizan en tiempo real.</p>
    </main>
  );
}
