...
import { MdArrowBack, MdCampaign, MdGroups, MdLocalActivity, MdPersonAddAlt1, MdRefresh, MdShoppingBag, MdStar, MdWarningAmber } from "react-icons/md";
  ...
  return (
    <main className="bonoa-shell min-h-screen pb-24">
      ...
      <section className="mt-6 rounded-[2rem] border border-orange-400/12 bg-orange-400/[0.04] p-5 sm:p-6">
        ...
        <div className="mt-2 flex flex-wrap items-end justify-between gap-4"><div><h2 className="text-2xl font-black text-white">Quién vuelve, quién compra y quién se está perdiendo.</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-zinc-500">Bonoa combina compras, visitas, consumos, recompensas y última actividad. El comercio trabaja con un código pseudónimo; no necesita ver email, nombre ni el QR del cliente.</p></div><Link href={`/business/${businessId}/growth`} className="brand-gradient inline-flex items-center gap-2 rounded-full px-5 py-3 text-xs font-black text-white"><MdCampaign size={18} /> Crear fidelización</Link></div>
      </section>
      ...
      {loading ? <div className="mt-7 h-72 animate-pulse rounded-[2rem] border border-white/8 bg-white/[0.03]" /> : <>
        ...
        {summary.atRisk > 0 || summary.loyal > 0 ? <section className="mt-5 grid gap-3 md:grid-cols-2">
          {summary.atRisk > 0 ? <Link href={`/business/${businessId}/growth`} className="rounded-[1.5rem] border border-red-400/15 bg-red-400/[0.045] p-5 transition hover:bg-red-400/[0.07]"><div className="flex items-center gap-3"><MdWarningAmber className="text-red-300" size={22} /><div><p className="text-sm font-black text-white">Recupera {summary.atRisk} cliente{summary.atRisk === 1 ? "" : "s"}</p><p className="mt-1 text-xs leading-5 text-zinc-500">Llevan más de 45 días sin actividad. Crea una campaña limitada para hacerlos volver.</p></div></div></Link> : null}
          {summary.loyal > 0 ? <Link href={`/business/${businessId}/growth`} className="rounded-[1.5rem] border border-amber-300/15 bg-amber-300/[0.04] p-5 transition hover:bg-amber-300/[0.07]"><div className="flex items-center gap-3"><MdStar className="text-amber-200" size={22} /><div><p className="text-sm font-black text-white">Ya tienes {summary.loyal} cliente{summary.loyal === 1 ? " fiel" : "s fieles"}</p><p className="mt-1 text-xs leading-5 text-zinc-500">Premia compras, visitas, gasto acumulado o consumos sin que el equipo tenga que llevar la cuenta.</p></div></div></Link> : null}
        </section> : null}
        ...
        <section className="mt-8">
          ...
          {visible.length ? <div className="mt-4 overflow-hidden rounded-[1.6rem] border border-white/8">
            <div className="hidden grid-cols-[1.05fr_.65fr_.8fr_.8fr_.8fr_.8fr] gap-3 border-b border-white/8 bg-white/[0.03] px-5 py-3 text-[10px] font-bold uppercase tracking-[0.13em] text-zinc-600 lg:grid"><span>Cliente</span><span>Segmento</span><span>Compras</span><span>Visitas</span><span>Premios</span><span>Última actividad</span></div>
            {visible.map((customer) => <article key={customer.wallet_id} className="grid gap-3 border-b border-white/8 px-5 py-4 last:border-0 lg:grid-cols-[1.05fr_.65fr_.8fr_.8fr_.8fr_.8fr] lg:items-center">
