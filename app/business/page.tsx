...
import { MdAddBusiness, MdArrowBack, MdArrowForward, MdGroups, MdOpenInNew, MdPointOfSale, MdRocketLaunch, MdStorefront } from "react-icons/md";
...
function BusinessHomeContent() {
  ...
  return (
    <main className="bonoa-shell min-h-screen pb-24">
        ...
        {!loading && businesses.map((business) => {
          ...
          return (
            <article key={business.id} className="bonoa-card relative overflow-hidden rounded-[1.8rem] p-5">
              ...
              {business.description ? <p className="mt-3 line-clamp-2 text-xs leading-5 text-zinc-500">{business.description}</p> : <p className="mt-3 text-xs text-zinc-700">Completa tu club, marca y primera fidelización.</p>}
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                {canManageTeam ? <Link href={`/business/${business.id}/onboarding`} className="inline-flex items-center gap-1.5 rounded-full border border-orange-400/15 bg-orange-400/[0.05] px-3 py-2 text-[10px] font-bold text-orange-200"><MdRocketLaunch size={14} /> Configurar club</Link> : null}
      ...
      {!loading && businesses.length === 0 ? <div className="mt-8 rounded-[1.6rem] border border-dashed border-white/10 p-8 text-center"><MdAddBusiness size={32} className="mx-auto text-zinc-600" /><p className="mt-3 text-sm font-bold text-white">Todavía no gestionas ningún negocio</p><p className="mt-2 text-xs leading-5 text-zinc-500">Crea el primero y Bonoa te llevará por un onboarding corto para dejar club, premio y QR listos.</p></div> : null}

      <section className="bonoa-card mt-8 rounded-[2rem] p-6 sm:p-8">
        <div className="max-w-xl"><p className="text-[10px] font-bold uppercase tracking-[0.24em] text-orange-300">Nuevo espacio</p><h2 className="mt-2 text-2xl font-black tracking-tight text-white">Crear negocio</h2><p className="mt-2 text-sm leading-6 text-zinc-500">Nombre e identificador y listo. Después eliges el tipo de fidelización desde una plantilla y Bonoa prepara el resto.</p></div>
